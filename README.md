...
`evaluate_classification` 函数专门用于分类评估，支持 F1 Score、AUC 和 Accuracy 指标：
...
print(f"F1 Score: {class_result['f1']:.4f}")
print(f"Accuracy: {class_result['accuracy']:.4f}")
...
### v0.4.4 (2026-02-27)
- **新增 Accuracy 指标**: 支持二分类、多分类的准确率计算
- **完善测试覆盖**: 新增 pytest 测试套件，覆盖检测、分类和统一接口
- **文档更新**: 修正 demo.py 为 examples.py，更新目录结构说明
- **开发体验**: 添加 dev 依赖 (pytest, pytest-cov)，配置 pytest
...
# 运行测试并生成覆盖率报告
pytest tests/ -v --cov=easyMetrics --cov-report=term-missing
```
